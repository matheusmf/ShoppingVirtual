...
public class LojaJpaController {
    ...
    public void create(Loja loja) {
        ...
        try {
            ...
            Locatario donoLoja = loja.getDonoLoja();
            if (donoLoja != null) {
                donoLoja = em.getReference(donoLoja.getClass(), donoLoja.getId());
                loja.setDonoLoja(donoLoja);
            }
            em.persist(loja);
            if (donoLoja != null) {
                Loja oldLojaOfDonoLoja = donoLoja.getLoja();
                if (oldLojaOfDonoLoja != null) {
                    oldLojaOfDonoLoja.setDonoLoja(null);
                    oldLojaOfDonoLoja = em.merge(oldLojaOfDonoLoja);
                }
                donoLoja.setLoja(loja);
                donoLoja = em.merge(donoLoja);
            }
    ...
    public void edit(Loja loja) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Loja persistentLoja = em.find(Loja.class, loja.getId());
            Locatario donoLojaOld = persistentLoja.getDonoLoja();
            Locatario donoLojaNew = loja.getDonoLoja();
            if (donoLojaNew != null) {
                donoLojaNew = em.getReference(donoLojaNew.getClass(), donoLojaNew.getId());
                loja.setDonoLoja(donoLojaNew);
            }
            loja = em.merge(loja);
            if (donoLojaOld != null && !donoLojaOld.equals(donoLojaNew)) {
                donoLojaOld.setLoja(null);
                donoLojaOld = em.merge(donoLojaOld);
            }
            if (donoLojaNew != null && !donoLojaNew.equals(donoLojaOld)) {
                Loja oldLojaOfDonoLoja = donoLojaNew.getLoja();
                if (oldLojaOfDonoLoja != null) {
                    oldLojaOfDonoLoja.setDonoLoja(null);
                    oldLojaOfDonoLoja = em.merge(oldLojaOfDonoLoja);
                }
                donoLojaNew.setLoja(loja);
                donoLojaNew = em.merge(donoLojaNew);
            }
            em.getTransaction().commit();
        } catch (Exception ex) {
